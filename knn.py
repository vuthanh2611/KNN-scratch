...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                for i_train in range(num_train):
                    # TODO: Fill dists[i_test][i_train]
                    dists[i_test][i_train]= np.sqrt(np.sum(np.square(X[i_test,:] - self.train_X[i_train,:])))
            return dists
    ...
    def compute_distances_one_loop(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                ...
                dists[i_test,:]= np.sqrt(np.sum(np.square(X[i_test,:] - self.train_X),axis=1))
            return dists
    ...
    def compute_distances_no_loops(self, X):
        ...
        if self.metric == 'euclidean':
            # TODO: Implement computing all distances with no loops!
            dists = np.sqrt((X**2).sum(axis=1).reshape(-1,1)+ (self.train_X**2).sum(axis=1).reshape(1,-1) - 2 * X.dot(self.train_X.T))
            dists=dists
            return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            y_close=[]
            sort_index=np.argsort(dists[i,:],axis=0)
            y_close=self.train_y[sort_index[:self.k]]
            
            pred[i] = np.argmax(np.bincount(y_close))
